...
async def run_agent_task(task_id: str, model: str, api_key: str, prompt: str, queue: asyncio.Queue):
    ...
    try:
        ...
        # Ensure result is a string before using replace
        if isinstance(result, dict):
            # If result is a dict, convert it to a JSON string
            result_str = json.dumps(result)
        else:
            # Otherwise convert to string directly
            result_str = str(result)
        
        # Format result to ensure it's in English
        result_str = result_str.replace("步骤", "Step").replace("终止", "Terminated").replace("达到了最大步数", "Maximum steps reached")
        
        # Send completion message
        await queue.put({
            "type": "success",
            "content": f"Task completed successfully: {result_str}"
        })
        
        # Wait for queue to process all messages
        await queue.join()
